...
def Check_Same(Kor_Name,Eng_Name,Apple_id):
    ...
    try:
        ...
        tmp_eng_name = tmp_eng_soup.find('h1',attrs={'class':'page-header__title'}).get_text().strip()

        tmp_kor_res = requests.get(kor_url,headers=headers)
        time.sleep(1)
        tmp_kor_soup = BeautifulSoup(tmp_kor_res.text,'lxml')
        tmp_kor_name = tmp_kor_soup.find('h1',attrs={'class':'page-header__title'}).get_text().strip()
